...
def GateInsertion(circ, c, qubit_reg): 
    """
    ...
    qubit_str = ""
    if (n_qubits == 1): qubit_str= qubit_reg+"[0]"
    elif (n_qubits == 2): qubit_str= qubit_reg+"[0]," + qubit_reg+ "[1]"
...
def GateInserter(scales, circuits, qubit_reg):
    scaled_circuits = []  
    ...
    for scale in scales:
        ...
        for circ in circuits:
            circ_ = circ.copy()
            scaled_circ = GateInsertion(circ_, scale, qubit_reg)
            scale_circuits.append(scaled_circ)
        scaled_circuits.append(scale_circuits)
